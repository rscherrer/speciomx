...
#' equilibrium
#' @param from,to Arguments to be passed to \code{find_singularities} (only
#' the bound towards which the initial selection gradient is pointing is
#' considered)
#' @param step Argument to be passed to \code{is_convergent}
#' @param ... Optional extra arguments for \code{find_singularities}
#'
...
#'
#' @seealso \code{get_gradient}, \code{find_singularities}, \code{is_stable},
#' \code{is_convergent}
#'
#' @examples
#'
...
get_endpoint <- function(
  ...
  # Is it attainable?
  is_convergent <- is_convergent(xend, pars, init, step)

  # Is is invasion-proof?
  is_stable <- is_stable(xend, pars, init)
